...
function apiDevPlugin(): Plugin {
  return {
    ...
    configureServer(server: ViteDevServer) {
      // Intercept /app and /app/* before Vite's static file middleware
      // to prevent EISDIR errors (the real /app directory exists on disk).
      server.middlewares.use((req, _res, next) => {
        if (req.url === "/app" || req.url?.startsWith("/app/") || req.url?.startsWith("/app?")) {
          // Let Vite's SPA fallback handle it — skip static file resolution.
          req.url = "/index.html";
        }
        next();
      });
